...
class AuthService {
  async getToken(login: string, password: string): Promise<string> {
    ...
    switch (res.status) {
      case 400:
        throw new Error(
          data?.error?.text ?? "Некорректный запрос к серверу авторизации",
        );
      case 401:
    ...
    if (!res.ok) {
      const errorText =
        ...
        "Неизвестная ошибка";
      throw new Error(
        `Ошибка сервера авторизации (${res.status}): ${errorText}`,
      );
    }
    ...
    if (!parsed.success)
      throw new Error("Сервер авторизации вернул невалидный ответ");
