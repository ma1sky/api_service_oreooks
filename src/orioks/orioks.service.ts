...
export type TimetableResponse = Record<
	string,
	[string, string]
>;
...
class OrioksApiService {
	private baseUrl = ORIOKS_LINK.replace(
		"http://",
		"https://",
	);
	...
	async getStudent(
		...
		return this.request<StudentResponse>(
			token,
			"/api/v1/student",
		);
	}
	...
	async getSemesterInfo(
		token: string,
	): Promise<SemesterInfoResponse> {
		return this.request<SemesterInfoResponse>(
			token,
			"/api/v1/schedule",
		);
	}
	...
	async getTimetable(
		...
		return this.request<TimetableResponse>(
			token,
	...
	async getLastUpdated(
		...
		return this.request<{
			...
			token,
			`/api/v1/schedule/groups/${groupId}/last_updated`,
		);
	}
