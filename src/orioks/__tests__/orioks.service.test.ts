...
describe("ORIOKS API Service", () => {
	...
	describe("Common API errors", () => {
		it("should throw 401 error for invalid token", async () => {
			...
			await expect(
				orioksApiService.getStudent(TOKEN),
			).rejects.toThrow(
		...
		it("should throw 404 error", async () => {
			...
			await expect(
				orioksApiService.getGroups(TOKEN),
			).rejects.toThrow(
		...
		it("should throw 410 error", async () => {
			...
			await expect(
				orioksApiService.getSemesterInfo(TOKEN),
			).rejects.toThrow(
		...
		it("should throw invalid Accept header error", async () => {
			...
			await expect(
				orioksApiService.getTimetable(TOKEN),
			).rejects.toThrow(
	...
	describe("getDisciplineEvents", () => {
		...
		it("should throw invalid discipline id error", async () => {
			...
			await expect(
				...
				"Дисциплины с данным идентификатором не существует",
			);
		});
	});
});